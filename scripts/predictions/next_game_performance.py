...
def predict_next_game_performance_linear_regression(
    spark: SparkSession, 
    df: DataFrame,
    series_name: str,
    platform: str = None
    ...
    filtered_data: DataFrame = df.filter(
        lower(col("title")).like(f"%{series_name.lower()}%")
    )

    if platform is not None:
        filtered_data = filtered_data.filter(
            lower(col("console")).like(platform.lower())
        )

    filtered_data = filtered_data.withColumn(
        "release_date_numeric", unix_timestamp("release_date")
    )
    ...
    if filtered_data.count():
        last_release_date_numeric: int = (
            filtered_data.select("release_date_numeric")
            .orderBy(col("release_date_numeric").desc())
            .first()[0]
        )
        ...
        print(f"Predicted PAL Sales: {predictions['pal_sales']}")
        print(f"Predicted Other Sales: {predictions['other_sales']}")


def train_and_predict(
    spark: SparkSession, 
    df: DataFrame,
    next_release_date_numeric: int, 
    seed=-42
):
    predictions = {}
    features_col = "release_date_numeric"
    targets = ["critic_score", "total_sales", "na_sales", "jp_sales", "pal_sales", "other_sales"]
    ...
    for target in targets:
        ...
        lr = LinearRegression(featuresCol="features", labelCol=target, regParam=0.1, solver="normal")
        ...
        next_data = spark.createDataFrame(
            [(next_release_date_numeric,)], [features_col]
        )
...
def predict_next_game_performance_random_tree(
    ...
    # Фильтрация данных для серии
    filtered_data = df.filter(lower(col("title")).like(f"%{series_name.lower()}%")).orderBy("release_date", ascending=True)
    ...
    target_columns = numeric_columns #["critic_score", "total_sales", "na_sales", "jp_sales", "pal_sales", "other_sales"]
